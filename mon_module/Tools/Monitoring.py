...
from mon_module.Tools.Drawing import draw_test_section, get_color_map_by_name
from mon_module.Tools.Utils import print_warning

SIZE_RATIO = 1.0 / (1024 * 1024)	# Passage de l'octet au Mega octet.
...
class Monitoring:
	...
	def _update_array_for_readability(self, round_time: int = 2):
		...
		self._memory = [m * SIZE_RATIO for m in self._memory]  # Passage en Mo
		self._disk = [(self._disk[i] - self._disk[i - 1]) * SIZE_RATIO for i in range(1, len(self._disk))]  # Passage en Mo et en delta d'utilisation
